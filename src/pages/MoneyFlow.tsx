...
function MoneyFlowInner() {
  ...
  const sectorSummary = flowData.map(d => ({
    ...
    sell: d.sell + 'T',
    net: (d.net > 0 ? '+' : '') + d.net + 'T',
    netColor: d.net > 0 ? '#22c55e' : '#f43f5e',
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: 12, padding: 16, background: th.cardBg, borderRadius: 12, border: `1px solid ${th.cardBorder}`, maxWidth: 1680, width: '100%', margin: '0 auto', height: 'fit-content' }}>
        ...
        <div style={{ borderBottom: `1px solid ${th.cardBorder}`, paddingBottom: 10 }}>
          ...
          <div style={{ display: 'flex', gap: 12, alignItems: 'center', fontSize: 10 }}>
            <div style={{ display: 'flex', gap: 4, alignItems: 'center' }}>
              <span style={{ background: '#22c55e', width: 12, height: 8, borderRadius: 2 }} />
              <span style={{ color: th.text }}>Mua NN</span>
            </div>
            <div style={{ display: 'flex', gap: 4, alignItems: 'center' }}>
              <span style={{ background: '#f43f5e', width: 12, height: 8, borderRadius: 2 }} />
          ...
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
            ...
            <div style={{ display: 'flex', gap: 4 }}>
              {(['1D', '1W', '1M'] as const).map((tf) => (
                <button key={tf} onClick={() => setTimeframe(tf)} style={{ background: timeframe === tf ? '#3b82f6' : th.inputBg, border: timeframe === tf ? '1px solid #2563eb' : `1px solid ${th.cardBorder}`, color: timeframe === tf ? '#fff' : th.textMuted, borderRadius: 3, padding: '3px 8px', fontSize: 9, fontWeight: 700, cursor: 'pointer' }}>{tf}</button>
              ))}
            </div>
          </div>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12 }}>
          <div style={{ background: th.cardBg, border: `1px solid ${th.cardBorder}`, borderRadius: 4, padding: 12, display: 'flex', flexDirection: 'column', gap: 8 }}>
            <h3 style={{ margin: 0, fontSize: 11, fontWeight: 700, color: '#22c55e', textTransform: 'uppercase' }}>💰 Mua NN</h3>
            <div style={{ flex: 1, overflowY: 'auto', display: 'flex', flexDirection: 'column', gap: 3 }}>
              {buyData.map((buy) => (
                <div key={buy.sector} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: 6, background: 'rgba(34,197,94,.1)', borderRadius: 3, fontSize: 9, borderLeft: '2px solid #22c55e' }}>
                  <span style={{ color: th.text, fontWeight: 700 }}>{buy.sector}</span>
                  <span style={{ color: '#22c55e', fontWeight: 700, fontFamily: "'JetBrains Mono', monospace" }}>+{buy.value}T</span>
                </div>
              ))}
            </div>
          </div>
          ...
          <div style={{ background: th.cardBg, border: `1px solid ${th.cardBorder}`, borderRadius: 4, padding: 12, display: 'flex', flexDirection: 'column', gap: 8 }}>
            <h3 style={{ margin: 0, fontSize: 11, fontWeight: 700, color: '#f43f5e', textTransform: 'uppercase' }}>📉 Bán NN</h3>
            <div style={{ flex: 1, overflowY: 'auto', display: 'flex', flexDirection: 'column', gap: 3 }}>
              {sellData.map((sell) => (
                <div key={sell.sector} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: 6, background: 'rgba(244,63,94,.1)', borderRadius: 3, fontSize: 9, borderLeft: '2px solid #f43f5e' }}>
                  <span style={{ color: th.text, fontWeight: 700 }}>{sell.sector}</span>
                  <span style={{ color: '#f43f5e', fontWeight: 700, fontFamily: "'JetBrains Mono', monospace" }}>-{sell.value}T</span>
                </div>
              ))}
            </div>
          </div>
              ...
              <tbody>
                {sectorSummary.map((sum) => (
                  <tr key={sum.sector} style={{ borderBottom: `1px solid ${th.cardBorder}`, height: 24 }}>
                    ...
                    <td style={{ padding: '4px 6px', textAlign: 'right', color: '#22c55e' }}>{sum.buy}</td>
                    <td style={{ padding: '4px 6px', textAlign: 'right', color: '#f43f5e' }}>{sum.sell}</td>
